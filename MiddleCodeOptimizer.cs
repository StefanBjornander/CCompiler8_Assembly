  ...
  public class MiddleCodeOptimizer {
    ...
    public void Optimize() {
      ...
      if (Start.CurrentFunction.Name.Equals("fileexists")) {
        string name = @"C:\Users\stefa\Documents\A A C_Compiler_CSharp_8_AssemblyX\C_Compiler_CSharp_8_Assembly\\" + Start.CurrentFunction.Name + "2.middlebefore";
...
//      CheckFloatingOld();
  ...
  // --------------------------------------------------------------------------
    ...
    private void RemoveTemporaryAssignX() {
      ...
      foreach (MiddleCode middleCode in m_middleCodeList) {
        if (middleCode.GetOperand(0) is Symbol resultSymbol) {
      ...
      foreach (MiddleCode outerCode in m_middleCodeList) {
        if (outerCode.Operator == MiddleOperator.Assign) {
          Symbol resultSymbol = (Symbol) outerCode.GetOperand(0),
                 assignSymbol = (Symbol) outerCode.GetOperand(1);
          ...
          if (resultSymbol.IsTemporary() &&
            ...
            for (int innerIndex = outerIndex + 1;
              ...
              if ((innerCode.GetOperand(0) == assignSymbol) && (minAssignIndex  == -1)) {
                minAssignIndex = innerIndex;
              }

              if (((innerCode.GetOperand(1) == resultSymbol) || (innerCode.GetOperand(2) == resultSymbol)) &&
            ...
            if ((minAssignIndex == -1) || (minAssignIndex >= maxAccessIndex)) {
              for (int innerIndex = outerIndex + 1; innerIndex < m_middleCodeList.Count; ++innerIndex) {
                ...
                if (innerCode.GetOperand(1) == resultSymbol) {
                  innerCode.SetOperand(1, assignSymbol);
                }
                if (innerCode.GetOperand(2) == resultSymbol) {
                  innerCode.SetOperand(2, assignSymbol);
                }
              }
